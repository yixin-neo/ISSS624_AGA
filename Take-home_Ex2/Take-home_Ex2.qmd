...
Further works in this exercise led me to realise that the YISHUN bus interchange is missing from the bus stop location shapefile. It has a postal code of 769100 and using the onemap geocoding function (describe in later section) will give us its SVY21 coordinates of 28314.3933093864, 45511.7310683895. The 'BUS_STOP_N' of Yishun Bus interchange is 59009. We will manually add them into the `busstop` df.
...
The point in black is to be added

![](images/yishun%20point.png){fig-align="center" width="408"}

**Adding geometry of Yishun Bus Interchange into `busstop` sf object.**

The code chunk below first creates a sf object containing only SVY21 geometry of the Yishun interchange. Next, it appends into the `busstop` df.
...
yishun <- data.frame(
  ...
  BUS_ROOF_N = NA, 
  LOC_DESC = 'YISHUN INT',
  geometry = st_sfc(st_point(c(28314.3933093864,45511.7310683895))))


yishun_sf <- st_sf(yishun, crs = 3414)

busstop <- rbind(busstop, yishun_sf)
```
...
`busstop_hex` : 5165 points suggesting all bus stops falls within at least one hexagon grid. This is good result because we are not losing any bus stops.
...
3.  `od_data` finally has 62,807 rows.
...
Let us rename the `od_data` to call it `flow_data` df. `flow_data` has 62,807 rows.
...
Filter to keep only inter-zonal trips, removing the intra-zonal trips. After filtering, the number of rows reduced from 62,807 to 62,186 rows.
...
Rows reduced from 62,807 to 62,186.
...
Let us take a look at the business locations (denoted by the black dots). The red hexagons highlights the grids with more than 50 business POI. We observe that business locations are more concentrated on the Southern parts of Singapore.
...
Let us take a look at the financial services locations (denoted by the black dots). The red hexagons highlights the grids with more than 50 financial services points. The financial services are most concentrated at the CBD district.
...
Before we proceed with regression methods, lets us check for multi-collinearity for all our continuous variables.

```{r}
#| fig-width: 14
#| fig-asp: 0.68
cor_matrix <- cor(flow_data1[, c("TRIPS","ORIGIN_TRAIN_EXIT_CT", "ORIGIN_PRESCHOOL_CT","ORIGIN_BUSINESS_CT","ORIGIN_FINSERV_CT","ORIGIN_SCH_CT","ORIGIN_RENTAL_CT","ORIGIN_1ROOM_CT","ORIGIN_2ROOM_CT","ORIGIN_3ROOM_CT","ORIGIN_4ROOM_CT","ORIGIN_5ROOM_CT","ORIGIN_EXEC_CT","ORIGIN_MULTI_CT","ORIGIN_STUDIO_CT","ORIGIN_OWNER_CT", "DESTIN_TRAIN_EXIT_CT","DESTIN_PRESCHOOL_CT","DESTIN_BUSINESS_CT","DESTIN_FINSERV_CT","DESTIN_SCH_CT","DESTIN_RENTAL_CT","DESTIN_1ROOM_CT","DESTIN_2ROOM_CT","DESTIN_3ROOM_CT","DESTIN_4ROOM_CT","DESTIN_5ROOM_CT","DESTIN_EXEC_CT","DESTIN_MULTI_CT","DESTIN_STUDIO_CT","DESTIN_OWNER_CT")])
...
The correlation heatmap shows that the origin and destination variable 'OWNER_CT' is highly corrrelated with '4ROOM_CT' and '5ROOM_CT'. For the subsequent regression methods, we will input either 'OWNER_CT' or '4ROOM_CT' and '5ROOM_CT' variables and use the variabels that give us the better result.

::: panel-tabset
### Unconstrained SIM

The final variables selected here are based on a series of trials and errors; they are statistically significant and would produce the best R-squared results. It turned out that using 'ORIGIN_4ROOM_CT' and 'ORIGIN_5ROOM_CT' produce better results than 'ORIGIN_OWNER_CT'.
...
The final variables selected here are based on a series of trials and errors; they are statistically significant and would produce the best R-squared results.
...
The R-squared value of 0.25 tells us that the explanatory variables are able to explain 25% of the variance in the TRIPS target variable.
...
Adjusted R-squared. As we increase the number of predictors, the R-square value naturally increases. Thus let us compare the adjusted R-square values instead. To do so, we write custom function:

![](images/Adj%20R%20squared%20formula.png){fig-align="center" width="212"}
...
Calc_AdjRSquared(CalcRSquared(uncSIM$data$TRIPS, uncSIM$fitted.values),
                 ...
                 length(uncSIM$coefficients)-1)
```

#### Unconstrained Coefficients

All the coefficients are statistically significant (refer to next tab). The most influential explanatory variables are *distance*, *destination train exit count* and *destination school count* with coefficients of -1.42, 0.44 and 0.28 respectively. This result is not surprising given the passengers would take bus to MRT stations or schools between 6am-9am on weekdays.
...
The R-squared value of 0.38 tells us that the explanatory variables are able to explain 38 % of the variance in the TRIPS target variable. It has higher R-squared value than the unconstrained model because it has additional 'knowledge' of the origin population data.
...
```{r}
Calc_AdjRSquared(CalcRSquared(orcSIM$data$TRIPS, orcSIM$fitted.values),
                 length(orcSIM$data$TRIPS),
                 length(orcSIM$coefficients)-1)
```

#### Origin Constrained Coefficients

All the coefficients are statistically significant (refer to next tab). The most influential explanatory variables are *distance*, *destination train exit count* and *destination school count* with coefficients of -1.50, 0.49 and 0.28 respectively. This result is consistent with unconstrained model.
...
```{r}
Calc_AdjRSquared(CalcRSquared(decSIM$data$TRIPS, decSIM$fitted.values),
                 length(decSIM$data$TRIPS),
                 length(decSIM$coefficients)-1)
```
...
The most influential explanatory variables are *distance*, *origin 5 room count* and *origin exec count* count with coefficients of -1.42, 0.09 and 0.04 respectively.
...
The R-squared value of 0.58 tells us that the explanatory variables are able to explain 58 % of the variance in the TRIPS target variable.

```{r}
CalcRSquared(dbcSIM$data$TRIPS, dbcSIM$fitted.values)
```

Adjusted R-squared of 0.57 is the highest for this model.

```{r}
Calc_AdjRSquared(CalcRSquared(dbcSIM$data$TRIPS, dbcSIM$fitted.values),
                 length(dbcSIM$data$TRIPS),
                 length(dbcSIM$coefficients)-1)
```

#### Doubly Constrained Coefficients

The explanatory variable 'distance' is statistically significant and has a coefficient of -0.158.
...
The output below reveals that **doubly constrained SIM** is the **best** model among all the four SIMs because it has the smallest RMSE value of 1039.
...
We can see that the points 'hug' more closely to the diagonal blue line, which indicates that doubly constrained model is indeed the best model. However, the origin constrained model is more useful in explaining the predictors that affects the flow of bus trips.

We focus our attention on the most influential factor affecting bus trips, that is count of MRT stations with the highest coefficient of 0.49. Given that I have prepared a MRT train flow data (in another exercise out of personal interest), we could incorporate this dataset into this exercise to visualise how commuters from various residential zones would potentially take bus to MRT stations.

```{r}
flowLine_train_wkdaymorn <- read_rds("data/rds/flowLine_train_wkday_am.rds")
```

In the map below, only bus trips above 13,000 and MRT trips above 14,800 trips are displayed. The lines represent bus flows while the pink lines represent MRT flows.
...
filtered_flowLine_bus <- flowLine %>% 
  filter(TRIPS >= 13000)

filtered_flowLine_train <- flowLine_train_wkdaymorn %>%
  filter(TRIPS >= 14800)

bs_count_filtered <- bs_count %>%
  filter(grid_id %in% c(filtered_flowLine_bus$ORIGIN_GRID_ID, filtered_flowLine_bus$DESTIN_GRID_ID))
...
The map above shows how the passengers commute from their residential areas to MRT stations via the yellow lines (Bus) and then from MRT to their final destinations via the pink lines (Train).
...
flow_data1 <- process_fitted_values(flow_data1, -resid(uncSIM, type='working'), "uncRES_norm")
flow_data1 <- process_fitted_values(flow_data1, -resid(orcSIM, type='working'), "orcRES_norm")
flow_data1 <- process_fitted_values(flow_data1, -resid(decSIM, type='working'), "decRES_norm")
flow_data1 <- process_fitted_values(flow_data1, -resid(dbcSIM, type='working'), "dbcRES_norm")
```
...
Since the best model is doubly-constrained, we filter to retain only the data points with over-predicted trips of above 20,000 and under-predicted trips of -20,000.
...
**Flows that are consistently under-predicted (in big magnitude) in all four models:**
...
**Flows that are consistently over-predicted (in big magnitude) in DEC and DBC models:**
...
Shades of Blue lines represents under-predicted flows while shades of red lines represents over-predicted flows.
...
tm_shape(bs_count_filtered) +
  ...
  tm_lines(col = 'uncRES',
           ...
           #scale= c(0.3, 3, 9, 15, 21, 30),
           palette = c("darkblue", "blue", "lightblue", "pink", "red", "darkred"),
           #palette = 'RdYlBu',
           n = 6,
           alpha= 0.9,
...
Across all the four models, common regions with high errors in predictions were Woodlands Check Point area, Yishun/Khatib interchange area, Chua Chu Kang interchange area and Boon Lay interchange area. This is because I have used the absolute residual count and these regions have the higher flows in our study area.
...
As compared to the original model, removing the outliers improved the model R-squared value from 0.58 to 0.60.
...
The coefficient of the distance variable is -1.58.
...
Four Spatial Interaction Models were built and compared to find out the important factors influencing the commuter flow in the Weekday Morning peak period. The R-squared values for the UNC, ORC, DEC and DBC models are 0.25, 0.38, 0.37 and 0.58 respectively. From the UNC and ORC models, the significant factors affecting commuter flow are **distance** (negative relationship), **destination** **MRT station exits count** and **destination** **school count** (in order of highest influence first). From the DEC model, the significant factors are **distance** and **origin** **5room flat dwellers count**.

When the outliers were removed, the R-squared value of the new DBC model increased to 0.60. Possible reason why the model could not predict flow of bus stops near Woodlands Checkpoint region could be that the number of people commuting between JB and Singapore daily for work purpose is really too huge and concentrated over Woodlands. We are also lacking the work opportunities information in JB side for model to predict properly.
...
1.  The Passenger Volume by Origin Destination Bus Stops data used does not include any transfer trip information. With our attractive and propulsive attributes, we can only assume that passengers take only 1 bus trip from source to their intended destination. With the inclusion of the MRT train station exits locations, we factor in the possibility of passenger commuting to MRT station for transfer.
