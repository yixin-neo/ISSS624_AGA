...
-   With reference to the time intervals provided in the table below, construct an O-D matrix of commuter flows for ONE time interval of ou rchoice by integrating *Passenger Volume by Origin Destination Bus Stops* and *Bus Stop Location* from [LTA DataMall](https://datamall.lta.gov.sg/content/datamall/en.html). The O-D matrix must be aggregated at the analytics hexagon level
...
**Load *MPSZ-2019* data**. It is a multipolygon sf object with 332 features / subzones/ units.
...
Before we can plot the base layer, we have to create hexagonal grids of 325 m (middle to edge) using the `mpsz` multipolygon sf object using `sf`.
...
4.  Filter to retain only hexagons with at least one subzone. The output `sz_count` has 1945 hexagon units.
...
`busstop_hex` : 5145 points suggesting all bus stops falls within at least one hexagon grid.
...
Next, we are going to append the grid_id from `busstop_hex` data frame onto `day_am` data frame. By doing so, we get the fields 'ORIGIN_GRID_ID' and 'ORIGIN_LOC_DESC' from `busstop_hex` into `day_am`.

We can achieve by performing left join between `day_am` and `busstop_hex` df twice.
...
We will perform another left join between `day_am` and `busstop_hex` df to get the 'DESTIN_GRID_ID' and the 'DESTIN_LOC_DESC' into `day_am` df. The number of rows before and after the left joins are the same.
...
Cheek for duplicates. The output shows that there are no duplicates.
...
[`spDists()`](https://www.rdocumentation.org/packages/sp/versions/2.1-1/topics/spDistsN1) of sp package will be used to compute the Euclidean distance between the centroids of the hexagonal grid_ids.
...
Recall `bs_count` contains 834 hexagons and a 834 x 834 distance matrix `dist` will be created.
...
### **Separating intra-flow from passenger volumn df**
...
```{r}
head(inter_zonal_flow,3) %>% knitr::kable()
```
...
`inter_zonal_flow` has no distance. `flow_data1` has distance data.
...
Perform a left join with od_data1 to get 'dist' information appended into `flowLine` df.
...
`flowLine` contains 62,120 rows but we will over print out trips \>= 5,000 in the datatable below.
...
In this map, we have filtered trips less than 5000 to make more manageable to analyse. Thicker line width represents more trips and the length of the lines represents the distance of each inter-hexagonal trip. In the next few tabs, we will perform further filtering on 'TRIPS' and 'dist' and plot interactive maps for us to get the trips details.
...
How to use the map: Click on the flowline of your interest, it will reveal the number of trips, origin bus stop descriptions and destination bus stop descriptions.
...
2\) Great volumes of flows were associated with *destinations* at bus or train stations, suggesting that passengers take bus for further transit. This gives us a hint that pressence of bus interchange / train stations could be an important dependent factor for 'TRIPS' in our Spatial Interaction Model later.
...
In this interactive map, in order to investigate the long distance bus trips with consderable amount of trips volume, we have filtered and retain TRIPS \>= 5000 and distance \>= 13km. We notice that these trips tend to connect passengers between the Northern parts of Singapore (Woodlands, Khatib and Yishun for instance) and the Eastern parts of Singapore (Tampanies, Seng Kang and Ponggol for instance). Taking a look at the Singapore MRT map, it would require passengers to use at least three different 'lines' in order for them to commute between Eastern and Northern Singapore.
...
In this map, we filter all trips with distances \>= 23 km irregardless of trips volume.
...
We will import the *Business* shape file provided by Prof Kam. It is a POINT sf object containing information about business locations in Sinapore. This variable is potentially one of the attractiveness of our destinations because passengers could take bus to work during the morning peak period.
...
We will import the *FinServ* shape file provided by Prof Kam. It is a POINT sf object containing information about financial service locations (Banks / Money Changers) in Sinapore. This variable is potentially one of the attractiveness of our destinations because passengers could take bus to work during the morning peak period.
...
We will use the [OneMap API](https://www.onemap.gov.sg/apidocs/apidocs/#search) to retrieve the longtitude (X) and latitude (Y) coordinates using the 'POSTAL_CODE' field. First load the `httr` package which works in a TIDY manner.
...
We will not be using commerical HDB property, therefore we will remove non-residential rows. The number of rows reduced from 12,442 to to 10,181.
...
Before: `hdb_sf` contanis 10,181 rows

After `st_intersection()`: `hdb_hex` contains 10,133 rows. Some points in hdb_sf do not fall within `bs_count` basic hexagon units. This is expected as `bs_count` that does not have hexagons with zero bustop counts.
...
'TRIPS' are all postive values, its distribution is highly skewed to the right.
...
All the coefficients are statistically significant. The most influential explanatory variables are *distance*, *destination train exit count* and *destination school count* with coefficients of -1.43, 0.45 and 0.29 respectively.
...
The most influential explanatory variables are *distance*, *destination train exit count* and *destination school count* with coefficients of -1.51, 0.49 and 0.29 respectively.
...
The explanatory variable distance has a coefficient of -0.159.
...
We can see that the points 'hugged' more closely towards the diagonal blue line, which indicates that doubly constrained model is indeed the best model.

### Visualising the residual errors

There are many types of [residuals](https://www.datascienceblog.net/post/machine-learning/interpreting_generalized_linear_models/) in glm() models like Response, Working, Pearson, Deviance residuals for instance. For simplicity, we will work with Response type, which is derived from 'actual' - 'predicted' values. In this exercise, I would like to calcualte 'Predicted - Actual' values instead; so that any over-prediction has positive values and under-predicted values will have a negative residual.

We will use the same function written previously to save the residual values as a dataframe and append that column of residual values to the flow_data1 df. We will append all the residual values of all the four SIMs.
...
Using the `highlight()` and `subplot()` functions of the `plotly` package, we will now print the 'residual vs predicted' plots side-by-side. The chart below offers a *coordinated view* of a selected point in any of the charts. We will use these coordinated charts to identify flows that are radically over and under predicted.
...
ll four models produces under-predicted (below X-axis) and over-predicted above X-axis) values. The unconstrained and origin-constrained model (top left) produces greater magnitude of under-predicted errors . The maximum number of actual trips is 75,364 and yet the unconstrained model produces only a maximum of about 15k trips.

18,341: Woodlands checkpoint to woodlands interchange

17,575: Woodlands checkpoint to Kranji MRT station

23,745: Woodlands Train to Woodlands Checkpoint

It seems like trips related to bus stops near the Woodlands Checkpoint tend to be poorly predicted.

### Visualising the predicted errors flows

Only flows with under-predicted values of more than 30,000 and over-predicted values of above 10,000 are visualised on the maps. Red-yellowish lines represents under-predicted flows while blue lines represents over-predicted flows.
...
tm_shape(bs_count) +
  ...
  tm_lines(col = 'uncRES',
           ...
           #col='blue') +
  tm_view(set.zoom.limits = c(11,17)) +
  tm_layout(main.title = 'Over-predicted O-D Flow for Unconstrained SIM' ,
            main.title.size = 1.0,
            main.title.fontface = 'bold')
unc
...
tm_shape(bs_count) +
  ...
  tm_view(set.zoom.limits = c(11,17)) +
  tm_layout(main.title = 'Over-predicted O-D Flow for Origin Constrained SIM',
            main.title.size = 1.0,
            main.title.fontface = 'bold')
...
tm_shape(bs_count) +
  ...
  tm_view(set.zoom.limits = c(11,17)) +
  tm_layout(main.title = 'Over-predicted O-D Flow for Destination Constrained SIM' ,
            main.title.size = 1.0,
            main.title.fontface = 'bold')
dec
...
tm_shape(bs_count) +
  ...
  tm_view(set.zoom.limits = c(11,17)) +
  tm_layout(main.title = 'Over-predicted O-D Flow for Doubly Constrained SIM',
            main.title.size = 1.0,
            main.title.fontface = 'bold')
dbc
