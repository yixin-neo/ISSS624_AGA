...
Drop the geometry because we need to process the attribute data. To get back the geometry properties, we can always left_join() `mpsz` df with our processed df again later.
...
Next, find the aggregate **sum** of the '*TRIPS*' field by the 'ORIGIN_SZ' in the `origin_data` df since we are plotting passenger trips originating at sub-zone level.
