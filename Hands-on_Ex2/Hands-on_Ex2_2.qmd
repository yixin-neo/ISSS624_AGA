...
Next, derives the spatially lagged variable of interest (i.e. GDPPC) using `lag.listw()` from the *spdep* package and centers the spatially lagged variable around its mean via subtraction.
...
These four command lines define the low-low (1), low-high (2), high-low (3) and high-high (4) categories. Comparing the local Moran's I values with its lag-variable values.

```{r}
quadrant[DV <0 & LM_I>0] <- 1 # LL
quadrant[DV >0 & LM_I<0] <- 2 # HL
quadrant[DV <0 & LM_I<0] <- 3 # LH
quadrant[DV >0 & LM_I>0] <- 4 # HH
```

| LM_I \> 0 : cluster of **similar** values.
| When DV \< 0 and LM_I \> 0, low values of spatial lag variables cluster together. Thus Low-low.
| When DV \> 0 and LM_I \> 0, high values of spatial lag-variables cluster together. Thus high-high.
| LM_I \<0 : cluster of **dissimilar** values.
| When DV \> 0 and LM \< 0, high values of spatial lag variables are among low ones, thus high-low.
| When DV \< 0 and LM_I \< 0 , low values of spatial lag variables are among the high ones, thus low-high.
